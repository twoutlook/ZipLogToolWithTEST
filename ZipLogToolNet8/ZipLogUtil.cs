...
namespace ZipLogTool
{
    public class ZipLogUtil
    {
        ...
        public void UnzipFiles(string ver, IniParser.IniData data, string logFilePath)
        {
            using (StreamWriter logWriter = new StreamWriter(logFilePath, true, Encoding.UTF8))
            {
                ...
                foreach (var path in pathsSection)
                {
                    ...
                    logWriter.WriteLine($"\n{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} Processing: {path.Key} => {path.Value} [Start Unzipping]");
                    ...
                    logWriter.WriteLine($"\n{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")} Processing: {path.Key} => {path.Value} [End Unzipping]\n");
                }
